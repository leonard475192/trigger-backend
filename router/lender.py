...
from usecases.car import create_car, get_car
...
def find_car(car_id: int, db: Session = Depends(get_db)):
    """
    Get car by id.
    args:
        id :
...
def add_rental(rental: lender.RentalCreateReq, db: Session = Depends(get_db)):
    ...
    db_rental = create_rental(db=db, rental=rental, locker_id=locker.id)
    print(db_rental.id)
